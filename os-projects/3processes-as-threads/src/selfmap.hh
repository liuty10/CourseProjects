...
public:
  ...
  bool isGlobals(std::string mainfile) const {
    ...
    // by a file (and all files have absolute paths)
	 ...
	 return (_readable && _writable && !_executable && _copy_on_write) && !isStack();
...
public:
  ...
  void getTextRegions() {
    for(const auto& entry : _mappings) {
      const mapping& m = entry.second;
      if(m.isText()) {
        if(m.getFile().find("libyithread") != std::string::npos) {
  ...
  void getGlobalRegions(regioninfo* regions, int* regionNumb) {
    ...
    for(const auto& entry : _mappings) {
      ...
      // skip libdoubletake
      if(m.isGlobals(_main_exe) && m.getFile().find("libyithread") == std::string::npos) {
        regions[index].start = (char*)m.getBase();
        regions[index].end = (char*)m.getLimit();
        index++;
      }
    }
